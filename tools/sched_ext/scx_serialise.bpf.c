...
const volatile u32 sched_group_strategy = SINGLE_GROUP; 
...
static inline bool can_dispatch(struct sched_job* job)
{
	...
	return (job->num_ready == job->num_alive && 
          (!(sched_group_strategy == SYZ_EID) || job->num_ready == job->num_total));
}

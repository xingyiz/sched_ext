...
static inline void update_pct_priority(pid_t pid, s32 new_prio)
{
	u32 index = (u32)pid % MAX_THREADS;
	u32 *prio_value = bpf_map_lookup_elem(&pct_priorities, &index);
	if (prio_value)
...
// static inline int serialise_get_pid_tgid(struct task_struct **p, pid_t *pid,
// 					 pid_t *tgid)
// {
...
void BPF_STRUCT_OPS(serialise_dequeue, struct task_struct *p, u64 deq_flags)
{
	...
	int status = tctx_map_insert(pid, 0, false, true);
	if (status < 0) {
		warn("[dequeue] failed to update tctx for pid: %d\n", pid);
		return;
	}
}
